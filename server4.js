...
//Populate list arrays

var employeeList = [];
var roleList = [];

function getEmployeeList(){pool.query(`SELECT id, first_name, last_name FROM employee`, (err, {rows})=>{
    let managers = rows;
...
function getRoleList(){pool.query(`SELECT id, title FROM role`, (err, {rows})=>{
    let titles = rows;
